...
import AddProduction from './AddProduct';
import ProductList from './ProductList';
import ShowProductList from '../ProductionManagement/Home/Components/ShowProductList';
...
const Layout = () => {
  ...
  return (
    <LayoutContainer className="flex flex-col flex-5 gap-5 h-full m-[20px] mt-0 ml-[10px]">
      ...
      <div className="">
        {/* Your button */}
        <div className="p-5 mb-5  bg-white rounded-2xl flex flex-col items-end">
          <button className="btn btn-primary" onClick={handleButtonClick}>
            {displayComponent1 ? <p>Add Product</p> : <p>View Products</p>}
          </button>
        </div>
        {/* Conditionally render component 1 or component 2 based on state */}
        {displayComponent1 ? <ShowProductList /> : <AddProduction />}
        {/* <Products /> */}
      </div>
    </LayoutContainer>
  );
