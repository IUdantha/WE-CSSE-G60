...
const EditSalaryForm = () => {
  ...
  const fetchData = async () => {
    try {
      const result = await axios.get('http://localhost:3000/admin-portal/salary-management/' + id, {
        headers: {
      ...
      formik.setValues({
        employeeId: result.data.salaryEmployeeID,
        employeeName: result.data.salaryEmployeeName,
        jobTitle: result.data.salaryJobTitle,
        role: result.data.salaryRole,
        basicSalary: result.data.salaryBasicSalary,
        netSalary: result.data.salaryNetSalary,
        status: result.data.salaryStatus
      });
    } catch (error) {
  ...
  const fetchIncrements = async () => {
    try {
      const response = await axios.get(API_URL + '/increment', {
        headers: {
          Authorization: `Bearer ${localStorage.getItem('token')}`,
          'Content-Type': 'application/json'
        }
