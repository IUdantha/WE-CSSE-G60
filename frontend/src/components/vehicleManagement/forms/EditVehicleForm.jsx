  ...
  const fetchData = async () => {
    try {
      const result = await axios.get(
        'http://localhost:3000/admin-portal/vehicle-management/' + id,
        {
          headers: {
      ...
      const convDate = new Date(result.data.vehicleBoughtDate);
      const regYear = new Date(result.data.vehicleRegisteredYear, 1, 1);
      formik.setValues({
        registrationNumber: result.data.vehicleRegNo,
        brand: result.data.vehicleBrand,
        model: result.data.vehicleModel,
        registerYear: regYear,
        owner: result.data.owner,
        category: result.data.vehicleCategory,
        boughtDate: convDate,
